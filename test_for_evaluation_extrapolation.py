...
#         data['landmarkwithpre']['pre'] = data['landmarkwithpre']['pre'].to(device)
        ...
        for interpolation_num in range(20):
            ...
            data['landmarkwithpre']['movement'] = surgical_movment_prediction * (interpolation_num / 10)
            ...
            os.makedirs(f'{save_path}/FAKE/{path}/movement', exist_ok=True)
            ...
            Image.fromarray(((np.clip(x_samples, -1, 1) + 1)/2 * 255).astype(np.uint8)).save(f'{save_path}/FAKE/{path}/movement/movement_{path}_FAKE_{interpolation_num:02}.png')

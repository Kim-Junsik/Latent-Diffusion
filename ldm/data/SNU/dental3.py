...
class DentalBase(Dataset):
    ...
    def __getitem__(self, idx):
        data = self.data[idx]
        landmark = np.load(data.replace('stylegan2_lat_png', 'stylegan2_lat_npy').replace('png', 'npy')).astype(np.float32)
        data = np.array(Image.open(data)) / 255.0
        ...
        return {'image': data[:,:,:1], 'landmark': landmark}
